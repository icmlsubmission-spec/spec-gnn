...
class DglGraphPropPredDatasetAugmentation(DglGraphPropPredDataset):
...
def read_graph_dgl(raw_dir, add_inverse_edge=False, additional_node_files=[], additional_edge_files=[], binary=False):
    ...
    for graph in tqdm(graph_list):
        ...
        g = basis_transform(g, norm=norm, epsilon=epsilon, power=power, identity=identity)
        # g = dgl.remove_self_loop(g)

        dgl_graph_list.append(g)
    total_time = time.time() - trans_start
    print('S^{\epsilon} transformation total and avg time:', total_time, total_time / len(graph_list))
