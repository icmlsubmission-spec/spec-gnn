...
def to_dense_adj(edge_index, batch=None, edge_attr=None, max_num_nodes=None):
    ...
    if edge_attr is None:
        edge_attr = torch.ones(edge_index.size(1), dtype=edge_index.dtype, device=edge_index.device)
    ...
    adj = torch.zeros(size, dtype=edge_index.dtype, device=edge_index.device)
