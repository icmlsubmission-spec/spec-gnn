...
def run_model(config):
    ...
    for i in tqdm(range(len(dataset)), desc="Pre-process"):
        ...
        dataset.graph_lists[i] = basis_transform(g, norm=norm, epsilon=epsilon, power=power, identity=identity)

    total_time = time.time() - trans_start
    print('S^{\epsilon} transformation total and avg time:', total_time, total_time / len(dataset))
    print("Bases total: {}".format(dataset.graph_lists[0].edata['bases'].shape[1]))
    # exit(0)
